...
file_path = os.path.join(script_dir, 'base_saved_results.pkl')

with open(file_path, 'rb') as f:
    basecifar = pickle.load(f)

file_path = os.path.join(script_dir, 'finp_saved_results_b01.pkl')

with open(file_path, 'rb') as f:
    finpcifar01 = pickle.load(f)

file_path = os.path.join(script_dir, 'finp_saved_results_b03.pkl')

with open(file_path, 'rb') as f:
    finpcifar03 = pickle.load(f)

file_path = os.path.join(script_dir, 'finp_saved_results_b005.pkl')

with open(file_path, 'rb') as f:
    finpcifar005 = pickle.load(f)
...
def loss_CoV(*series, labels=None):
    ...
    # Output CoV stats
    print('loss cv')
    print(', '.join([f'{np.mean(c):.3f}' for c in cov_results]))
    print('\n')
    plt.close(fig)
...
def loss_fi(*series, labels=None):
    ...
    print('loss fi')
    print(', '.join([f'{np.mean(f):.3f}' for f in fi_results]))
    print('\n')
    plt.close(fig)
...
#############CIFAR alpha ############
# # set labels to all results pickle files
# LABEL = ['Baseline', r'$\beta$=0.05', r'$\beta$=0.1', r'$\beta$=0.3']


# # line_average_train_accuracy(), line_average_test_accuracy() for training/testing accuracy figure.
# line_average_train_accuracy(basecifar[1], finpcifar005[1],finpcifar01[1],finpcifar03[1], labels=LABEL)
# line_average_test_accuracy(basecifar[2], finpcifar005[2],finpcifar01[2],finpcifar03[2], labels=LABEL)

# # loss_CoV(), loss_fi() for CoV and FI of Loss figure.
# loss_CoV(basecifar[3], finpcifar005[3],finpcifar01[3],finpcifar03[3],
#                               labels=LABEL)
# loss_fi(basecifar[3], finpcifar005[3], finpcifar01[3],finpcifar03[3],
#                               labels=LABEL)

# # siacvplot(), siafiplot() for CoV and FI of SIA figure.
# siacvplot(basecifar[0], finpcifar005[0], finpcifar01[0],finpcifar03[0],
#                              labels=LABEL, mode='CoV(SIA)')
# siafiplot(basecifar[0], finpcifar005[0], finpcifar01[0], finpcifar03[0],
#                             labels=LABEL, mode='FI(SIA)')

# # sia_rounds() for average SIA accuracy across round figure.
# sia_rounds(basecifar[4], finpcifar005[4], finpcifar01[4], finpcifar03[4], labels=LABEL)

# # line_eod() for EOD figure.
# line_eod(basecifar[0], finpcifar005[0], finpcifar01[0], finpcifar03[0],labels=LABEL)
